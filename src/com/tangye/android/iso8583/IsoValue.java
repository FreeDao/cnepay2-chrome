...
public class IsoValue<T> implements Cloneable {
	...
	public IsoValue(IsoType t, T value) {
		...
		if (type == IsoType.LLVAR || type == IsoType.LLLVAR || type == IsoType.LLVARBCD || type == IsoType.LLLVARBCD) {
			...
			if ((t == IsoType.LLVAR || t == IsoType.LLVARBCD) && length > 99) {
				throw new IllegalArgumentException("LLVAR can only hold values up to 99 chars");
			} else if ((t == IsoType.LLLVAR || t == IsoType.LLLVARBCD) && length > 999) {
	...
	public IsoValue(IsoType t, T val, int len) {
		...
		} else if (t == IsoType.LLVAR || t == IsoType.LLLVAR || t == IsoType.LLVARBCD || t == IsoType.LLLVARBCD) {
			// 变长忽略 len 的输入
	...
	public String toString() {
		...
		if (type == IsoType.NUMERIC || type == IsoType.AMOUNT) {
			...
			return type.format(value.toString(), length);
		} else if (type == IsoType.LLLVAR || type == IsoType.LLLVAR || type == IsoType.LLLVARBCD || type == IsoType.LLLVARBCD) {
	...
	public void write(OutputStream outs, boolean binary) throws IOException {
		if (type == IsoType.LLLVAR || type == IsoType.LLVAR || type == IsoType.LLLVARBCD || type == IsoType.LLVARBCD) {
			if (binary) {
				...
				if(type == IsoType.LLLVARBCD || type == IsoType.LLVARBCD) {
	                // BCD needs to encode the value for BCD type
	                byte[] buf = new byte[(length / 2) + (length % 2)];
	                toBcd(toString(), buf, true);
	                outs.write(buf);
	                return;
	            }
			} else {
		...
		outs.write(toString().getBytes("ISO-8859-1"));
	}
	...
	private void toBcd(String value, byte[] buf, boolean isleft) {
		int charpos = 0; //char where we start
		int bufpos = 0;
		if (!isleft && value.length() % 2 == 1) {
			//for odd lengths we encode just the first digit in the first byte
			buf[0] = (byte)(value.charAt(0) - 48);
			charpos = 1;
			bufpos = 1;
		} else if (isleft && value.length() % 2 == 1) {
		    //for odd lengths we just add a "0" to the end of original value
		    value += "0";
        }
