...
ProgressDialog.OnCancelListener{
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if(intent.getStringArrayExtra("allString") == null) {
	...
	public void onClick(View v) {
		...
		case R.id.upload_ticket:
			...
			if(null == pathSD)
			{
				Toast.makeText(SignNameActivity.this, "没有SD卡无法保存", Toast.LENGTH_LONG).show();
				progressDialog.cancel();
				finish();
				return;
			}
			...
			consumeHistory = SavePath + "/consumeHistory/";
			...
			PicDraw pic = new PicDraw(SignNameActivity.this, allString);
			if (pic.isDrawingCacheEnabled() == false) {
				pic.setDrawingCacheEnabled(true);
			}
			Bitmap drawingCache = Bitmap.createBitmap(290, 540, Bitmap.Config.RGB_565);
			...
			try { 
				if (bos.size() > 0) {
					file = new File(consumeHistory + "/" + allString[12] + ".png");
					if (!file.exists()) {  
			            file.createNewFile();
			        }
			...
			new Thread(TAG) {
				public void run() {
					...
					String err = "";
					File upfile = null;
					upfile = new File(consumeHistory + "/" + allString[12] + ".png");
					...
					try {
			        	...
			        	//reqEntity.addPart("merchantNo", new StringBody(POS.getPOSDecrypt(POS.USERMARK)));
			    		reqEntity.addPart("transId", new StringBody(allString[10]));
